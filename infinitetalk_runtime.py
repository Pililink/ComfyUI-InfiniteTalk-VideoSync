...
class InfiniteTalkEngine:
    ...
    def _load_models(self):
        ...
        with _timed_log("Load diffusion model", model=self.config["wan_model"]):
            self.model = _invoke(
                ...
                attention_mode=self.config.get("attention_mode", "sageattn"),
    ...
    def render_segment(
        ...
        with _timed_log("Segment clip encode", segment=segment_label):
            clip_embeds = _invoke(
                ...
                strength_2=float(self.config.get("clip_strength_2", 0.7)),
                crop="center",
