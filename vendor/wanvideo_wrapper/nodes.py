...
def _get_text_embed_cache_dir():
    try:
        comfy_root = os.path.abspath(os.path.join(folder_paths.models_dir, os.pardir))
        return os.path.join(
            comfy_root,
            "cache",
        ...
        return os.path.join(script_directory, 'text_embed_cache')


cache_dir = _get_text_embed_cache_dir()
...
class WanVideoTextEncodeCached:
    ...
    def INPUT_TYPES(s):
        return {"required": {
            ...
            "use_disk_cache": ("BOOLEAN", {"default": True, "tooltip": "Cache the text embeddings to disk for faster re-use, under the ComfyUI cache/ComfyUI-InfiniteTalk-VideoSync/text_embed_cache directory"}),
    ...
    def INPUT_TYPES(s):
        return {"required": {
            ...
            "optional": {
                ...
                "use_disk_cache": ("BOOLEAN", {"default": False, "tooltip": "Cache the text embeddings to disk for faster re-use, under the ComfyUI cache/ComfyUI-InfiniteTalk-VideoSync/text_embed_cache directory"}),
                "device": (["gpu", "cpu"], {"default": "gpu", "tooltip": "Device to run the text encoding on."}),
            }
        }
